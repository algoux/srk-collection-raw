...
(function() {
    ...
    class CSGSwitch {
        ...
        this.defaultOptions = {
            ...
            /** pill：圆角轨道 + 圆形滑块（默认）；square：方角轨道 + 方形滑块 */
            shape: 'pill',
            animate: false,       // 略长过渡；默认可避免首屏多余动效
            animation: 'default', // default | advanced | elastic（与 animate 叠加）
        ...
        parseOptions(switchEl, options) {
            ...
            // 从 wrapper 已有 class 检测 size / theme（HTML 可直接写 class 而非 data-*）
            const wrapper = switchEl.closest('.csg-switch');
            if (wrapper) {
                for (const sz of ['sm', 'md', 'lg']) {
                    if (wrapper.classList.contains('csg-switch-' + sz)) { config.size = sz; break; }
                }
                for (const th of ['success', 'warning', 'danger', 'info']) {
                    if (wrapper.classList.contains('csg-switch-' + th)) { config.theme = th; break; }
                }
            }

            // 从 data 属性读取配置（优先级高于 class）
            ...
            const dataShape = switchEl.dataset.csgShape;
            ...
            if (dataShape === 'square' || dataShape === 'pill') config.shape = dataShape;
        ...
        calculateTrackWidth(track, config) {
            ...
            const sliderRem = { sm: 0.875, md: 1.125, lg: 1.3125 }[config.size] || 1.125;
            const gapRem = { sm: 0.15, md: 0.2, lg: 0.22 }[config.size] || 0.2;
            const hpadRem = 0.3;
            const textInsetRem = 0.22;
            const totalWidth =
                maxWidth + (sliderRem + gapRem + textInsetRem + 2 * hpadRem) * 16;

            track.style.width = `${Math.ceil(totalWidth)}px`;
        }
        ...
        applyConfig(switchEl, config) {
            ...
            if (config.animation && config.animation !== 'default') {
                wrapper.classList.add(`csg-switch-${config.animation}`);
            }
            if (config.shape === 'square') {
                wrapper.classList.add('csg-switch-shape-square');
            } else {
                wrapper.classList.remove('csg-switch-shape-square');
            }
        }
        ...
        /**
         * 视觉与 title 与 checkbox 对齐（滑块位置由 CSS :checked 控制）
         */
        updateSwitchState(switchEl, config) {
            const track = switchEl.nextElementSibling;
            if (track) {
                track.classList.toggle('csg-switch-active', !!switchEl.checked);
            }
            if (config) {
                this.updateTitle(switchEl, config);
            }
        ...
        syncSwitchState(switchEl) {
            ...
            this.updateSwitchState(switchEl, this.getConfig(switchEl));
        }
        ...
        destroy(switchEl) {
            ...
            if (wrapper) {
                ...
                wrapper.classList.remove(
                    'csg-switch', 'csg-switch-sm', 'csg-switch-md', 'csg-switch-lg',
                    'csg-switch-primary', 'csg-switch-success', 'csg-switch-warning',
                    'csg-switch-danger', 'csg-switch-info', 'csg-switch-animate',
                    'csg-switch-advanced', 'csg-switch-elastic', 'csg-switch-premium',
                    'csg-switch-glass', 'csg-switch-loading', 'csg-switch-shape-square'
                );
            }
        }
    }
